...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $doctor != '') {
    $sql = "SELECT * FROM records WHERE doctor = '$doctor'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $records = $result->fetch_all(MYSQLI_ASSOC);
    } else {
        $error_message = "No records found for the selected doctor.";
    }
}

$sqlDoctors = "SELECT DISTINCT doctor FROM records";
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        select, input[type="submit"] { padding: 10px; font-size: 1em; border: 1px solid #ddd; border-radius: 5px; }
...
<div class="form-container">
    <form action="" method="POST">
        ...
        <select id="doctor" name="doctor">
            ...
            <?php while ($row = $doctors->fetch_assoc()) { ?>
                <option value="<?php echo $row['doctor']; ?>" <?php echo ($doctor == $row['doctor']) ? 'selected' : ''; ?>><?php echo $row['doctor']; ?></option>
            <?php } ?>
        </select>
    ...
    <?php } elseif (!empty($doctor) && empty($records)) { ?>
        <p>No records found for the selected doctor.</p>
    <?php } ?>
</div>
